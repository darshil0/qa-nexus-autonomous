...
- **`args[key]` → `args[key] ?? ''`**: `args` is typed `Record`, but a caller omitting a parameter leaves `args[key]` as `undefined` at runtime. This `undefined` was silently spread into `skill.execute()`, which has signature `(...args: string[])`. The guard makes the fallback explicit and type-safe.
...
- **Component Refactoring**: Migrated all tab components to the new design system: `OrchestratorTab`, `Agent1Tab`, `Agent2Tab`, `Agent3Tab`, `ReportsTab`.
...
- Moved 15 source files from root to `src/` directory: core files (`App.tsx`, `index.tsx`, `constants.ts`, `types.ts`, `metadata.json`).
